...
int set_addr(struct sockaddr_in *addr, 
	...
	if (name != NULL){
		...
		if(h == NULL)
			return -1;
		addr->sin_addr_.s_addr =
			*(u_int32_t *)h->h_addr_list[0];
	}else
		addr->sin_sddr.s_addr = htonl(inaddr);
...
int stream_read(int sockfd, char *buf, int len){
	...
	while(remaining > 0){
		if((nread = read(sockfd, buff, remaining))
			== -1)
			return -1;
		if(nread == 0)
			break;
...
int main(int argc, char *argv[]) {
	...
	if ( nread < 0 ) {
		printf( ” E roa re l a c i t i r e a din f i s i e r \n” ) ;
		exit(1);
	}
	close(sockfd);
	close(fd);
	
	printf( ” F i s i e r u l a f o s t t r i m i s cu s u c c e s \n” ) ;
	exit(0);
}
